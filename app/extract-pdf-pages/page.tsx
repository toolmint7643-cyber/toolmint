...
type ExtractMode = "single-pdf" | "separate-pages" | "separate-ranges";
    ...
    .forEach((part) => {
      if (part.includes("-")) {
        ...
        if (rangePages.length) {
          groups.push({ label: `pages-${from}-${Math.min(to, totalPages)}`, pages: rangePages });
        }
...
function buildConsecutiveGroups(pages: number[]) {
...
export default function ExtractPdfPagesPage() {
  ...
  const [pageInput, setPageInput] = useState("1-3,7,10");
  const [selectedPages, setSelectedPages] = useState<number[]>([]);
  const [rangeGroups, setRangeGroups] = useState<PageGroup[]>([]);
  ...
  const [status, setStatus] = useState("Upload PDF files and choose pages to extract.");
  ...
  const outputDescription =
    extractMode === "single-pdf"
      ? "One PDF will be created"
      : extractMode === "separate-pages"
        ? `${selectedPages.length} separate PDF file${selectedPages.length === 1 ? "" : "s"} will be created`
        : `${buildConsecutiveGroups(selectedPages).length} range PDF file${buildConsecutiveGroups(selectedPages).length === 1 ? "" : "s"} will be created`;
  ...
  function clearAll() {
    ...
    setRangeGroups([]);
    setThumbnails([]);
    setStatus("Upload PDF files and choose pages to extract.");
  ...
  function applyPageSelection(mode: PageMode) {
    ...
    if (mode === "custom") {
      ...
      if (!parsed.pages.length) {
        setError("Please enter at least one valid page.");
        return;
      }

      setSelectedPages(parsed.pages);
      setRangeGroups(parsed.groups);
      return;
    }

    const quickPages = getQuickPages(mode, activeTotalPages);
    setSelectedPages(quickPages);
    setRangeGroups(buildConsecutiveGroups(quickPages));
  }
  ...
  function togglePage(page: number) {
    ...
    setSelectedPages((current) => {
      ...
      setRangeGroups(buildConsecutiveGroups(next));
      return next;
    });
  }
  ...
  function selectAllPages() {
    ...
    const pages = Array.from({ length: activeTotalPages }, (_, index) => index + 1);
    setSelectedPages(pages);
    setRangeGroups(buildConsecutiveGroups(pages));
  }
  ...
  function invertSelection() {
    ...
    const inverted = Array.from({ length: activeTotalPages }, (_, index) => index + 1).filter((page) => !selected.has(page));

    setSelectedPages(inverted);
    setRangeGroups(buildConsecutiveGroups(inverted));
  }
  ...
  async function createPdfFromPages(sourcePdf: PDFDocument, pages: number[]) {
  ...
  async function processSinglePdf(item: PdfItem) {
    ...
    if (!normalizedSelected.length) {
      throw new Error("No valid pages selected for this PDF.");
    }
    ...
    if (extractMode === "single-pdf") {
      const blob = await createPdfFromPages(sourcePdf, normalizedSelected);
    ...
    if (extractMode === "separate-pages") {
      for (const page of normalizedSelected) {
        const blob = await createPdfFromPages(sourcePdf, [page]);
        const url = URL.createObjectURL(blob);

        files.push({
          id: createId(),
          name: `${baseName}-page-${page}.pdf`,
          size: formatBytes(blob.size),
          url,
          blob,
        });
    ...
    const groups = rangeGroups.length ? rangeGroups : buildConsecutiveGroups(normalizedSelected);

    for (const group of groups) {
      const groupPages = group.pages.filter((page) => page >= 1 && page <= pageCount);

      if (!groupPages.length) continue;

      const blob = await createPdfFromPages(sourcePdf, groupPages);
      const url = URL.createObjectURL(blob);
  ...
  async function processPdfs() {
    ...
    if (!selectedPages.length) {
      setError("Select pages to extract first.");
      return;
    }

    clearOutputs();
    setIsProcessing(true);
    setStatus("Extracting pages from PDFs...");
    ...
    for (const item of validFiles) {
      ...
      try {
        const files = await processSinglePdf(item);
        createdOutputs.push(...files);

        setPdfItems((current) =>
          current.map((pdf) => (pdf.id === item.id ? { ...pdf, status: "success", error: "" } : pdf))
        );
      } catch (processError) {
        console.error("Extract PDF pages error:", processError);
    ...
    setStatus(`Finished. ${createdOutputs.length} extracted PDF file${createdOutputs.length === 1 ? "" : "s"} ready.`);
    setIsProcessing(false);

    if (createdOutputs.length) {
      alert("PDF pages extracted successfully.");
    }
  }

  async function downloadAll() {
    if (!outputs.length) {
      alert("No extracted PDFs to download.");
      return;
    }
    ...
    link.href = url;
    link.download = "toolmint-extracted-pdfs.zip";
    link.click();
  ...
  async function copySummary() {
    const summary = `Extract PDF Pages Summary

Mode: ${extractMode}
Selected pages: ${selectedText}
Pages selected: ${selectedPages.length}
Source active PDF pages: ${activeTotalPages}
    ...
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 text-white">
        <PageTitle
          ...
          description="Extract pages from PDF online for free. Select pages visually, use ranges, extract into one PDF, separate pages or separate ranges with browser-side privacy."
        />
          ...
          <div className="space-y-6">
            <ToolCard>
              ...
              >
                ...
                <span className="mt-1 text-sm text-slate-300">Batch extraction supported. Files stay in your browser.</span>
                  ...
                  {pdfItems.map((item) => (
                    <div
                      ...
                      className={`rounded-2xl border p-4 transition ${
                        activeId === item.id ? "border-blue-500 bg-blue-950/30" : "border-slate-700 bg-slate-900"
                      }`}
                    >
            ...
            <ToolCard>
              <h2 className="mb-4 text-2xl font-bold text-white">⚙️ Extraction Settings</h2>

              <label className="block">
                <span className="mb-2 block text-sm font-semibold text-slate-300">Extraction Mode</span>
                <select
                  value={extractMode}
                ...
                >
                  <option value="single-pdf">Extract into one PDF</option>
                  <option value="separate-pages">Extract each page separately</option>
                  <option value="separate-ranges">Extract ranges separately</option>
                </select>
              </label>
          ...
          <div className="space-y-6">
            <ToolCard>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-purple-500/40 bg-purple-950/30 p-4">
                  <p className="text-sm text-purple-200">Output</p>
                  <p className="mt-2 text-lg font-bold text-purple-300">{extractMode.replace("-", " ")}</p>
                </div>
              </div>
                      ...
                      return (
                        <button
                          ...
                          className={`relative rounded-xl border p-2 transition ${
                            selected ? "border-emerald-500 bg-emerald-950/40" : "border-slate-700 bg-slate-900 hover:border-blue-500"
                          }`}
                        >
                          ...
                          {selected && (
                            <span className="absolute right-2 top-2 rounded-full bg-emerald-600 px-2 py-1 text-xs font-bold text-white">
                              Extract
                            </span>
                          )}
                        </button>
            ...
            <ToolCard>
              ...
              <div className="rounded-2xl border border-slate-700 bg-slate-900 p-4">
                <p className="font-bold text-white">Extraction summary</p>
                <p className="mt-2 text-sm text-slate-300">Selected pages: {selectedText}</p>
                <p className="mt-3 text-sm font-semibold text-emerald-300">{outputDescription}</p>
              </div>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          ...
          <ToolCard>
            ...
            <p className="text-slate-300">
              ...
              ZIP download is generated inside your browser.
            </p>
          </ToolCard>
        </div>
      </main>
