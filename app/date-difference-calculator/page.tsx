...
export default function DateDifferenceCalculatorPage() {
  ...
  const result = useMemo(() => {
    ...
    const totalDays =
      Math.floor((to.getTime() - from.getTime()) / MS_PER_DAY) + extraDay;
    const calendar = getCalendarDifference(
      from,
      includeEndDate ? addDays(to, 1) : to
    );
  ...
  const operationResult = useMemo(() => {
    ...
    return operationMode === "add"
      ? addDays(base, safeDays)
      : addDays(base, -safeDays);
  }, [operationBaseDate, operationDays, operationMode]);

  const quickExamples = [
    {
      label: "Today to 30 days",
      start: todayValue(),
      end: dateInputValue(addDays(new Date(), 30)),
    },
    { label: "This Year", start: "2026-01-01", end: "2026-12-31" },
    {
      label: "Next 90 Days",
      start: todayValue(),
      end: dateInputValue(addDays(new Date(), 90)),
    },
    {
      label: "Project Sprint",
      start: todayValue(),
      end: dateInputValue(addDays(new Date(), 14)),
    },
  ];
        ...
        <ToolCard>
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-[0.95fr_1.05fr]">
              ...
              <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
                ...
                <div className="rounded-2xl border border-blue-700 bg-blue-950/30 p-6 text-center">
                  ...
                  <div className="mt-3 break-words text-4xl font-extrabold text-blue-300">
                    {result.calendar.years}y {result.calendar.months}m{" "}
                    {result.calendar.days}d
                  </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setOperationBaseDate(event.target.value)
                    }
                    className="w-full rounded-xl border border-slate-700 bg-slate-800 p-4 text-white outline-none focus:border-blue-500"
                  />
                </label>
            ...
            <div className="rounded-2xl border border-yellow-700 bg-yellow-950/30 p-5 text-sm text-yellow-100">
              Date calculations use calendar days in your browser timezone. For
              payroll, legal, banking or official deadline calculations, always
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
                ...
                <p className="text-slate-300">
                  A date difference calculator finds the number of days, weeks,
                  months and years between two dates. It is useful for project
                  timelines, travel plans, deadlines, age gaps and event
                  countdowns.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
                ...
                <p className="text-slate-300">
                  Use this tool to calculate days between dates, weekdays
                  between dates, weekends, date duration, business planning
                  ranges and add or subtract days from a selected date.
                </p>
              </div>
            </div>
          </div>
